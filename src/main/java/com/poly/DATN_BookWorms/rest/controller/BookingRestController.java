...
public class BookingRestController {
	...
	public ResponseEntity<byte[]> generatePdf(@PathVariable String bookingid) {
		try {
			...
			document.add(new Paragraph(detailbookings.get(0).getBooks().getShoponlines().getShopname().toUpperCase() + "STORE", titleFont));
			...
			document.add(new Paragraph("----------------------------------------", contentFont));
			document.add(new Paragraph("Địa chỉ: Phường Nhà Mát, Thành phố Bạc Liêu, Bạc Liêu", contentFont));
			document.add(new Paragraph("Số điện thoại: 0378977287", contentFont));
			document.add(new Paragraph("----------------------------------------", contentFont));
			document.add(new Paragraph("Sản phẩm             Số lượng      Giá", contentFont));
			document.add(new Paragraph("----------------------------------------", contentFont));
			for (Detailbookings detailbooking : detailbookings) {
				document.add(new Paragraph(detailbooking.getBooks().getBookname()+"          "+detailbooking.getQuantity()+"                  "+detailbooking.getBooks().getPrice(), contentFont));
			}
			document.add(new Paragraph("----------------------------------------", contentFont));
			document.add(new Paragraph("Tổng cộng:                             "+bookings.getCost(), contentFont));
			document.add(new Paragraph("----------------------------------------", contentFont));
			document.add(new Paragraph("Cảm ơn quý khách đã mua hàng!", contentFont));
			document.close();
